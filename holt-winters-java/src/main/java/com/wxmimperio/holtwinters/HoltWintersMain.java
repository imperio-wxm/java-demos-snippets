...
/*        System.out.println(y.length);
        ...
        HoltWintersModel holtWintersModel =
                new HoltWintersModel(0.049999997, 0.95, 0.07, 288, HoltWintersModel.SeasonalityType.MULTIPLICATIVE, false);
        //=System.out.println(Arrays.toString(holtWintersModel.doPredict(Arrays.asList(dd), 3)));

        inputData();


        List<Double> value = originData.stream().map(HoltWintersWeka2.OriginData::getValue).collect(Collectors.toList());
    ...
    private static long genTime(int index, int stepMin) throws Exception {
        ...
        calendar.setTime(dateTimeFormat.parse("2021-01-09 00:00"));
