...
public class JsonSyslogUtils {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JsonSyslogUtils.class);
    ...
    public static Set<String> chooseFieldsToKeep(String keepFields) {
        ...
        if (keepFields.equals("true") || keepFields.equals("all")) {
            Set<String> fieldsToKeep = new HashSet<>(1);
            fieldsToKeep.add(KEEP_FIELDS_ALL);
            return fieldsToKeep;
        }

        Set<String> fieldsToKeep = new HashSet<>(DEFAULT_FIELDS_TO_KEEP.length);
    ...
    public JsonSyslogUtils() {
        this(false);
    }

    public JsonSyslogUtils(boolean isUdp) {
        this(DEFAULT_SIZE, new HashSet<String>(Arrays.asList(SyslogSourceConfigurationConstants.DEFAULT_KEEP_FIELDS)), isUdp);
    }

    public JsonSyslogUtils(Integer eventSize, Set<String> keepFields, boolean isUdp) {
        this.isUdp = isUdp;
        isBadEvent = false;
    ...
    JsonEvent buildEvent() {
        try {
            ...
            if (isBadEvent) {
                logger.debug("Event created from Invalid Syslog data.");
            ...
            // format the message
            return new JsonEvent(new String(body), headers);
        } finally {
            reset();
        }
    }
    ...
    private void formatHeaders() {
        ...
        for (SyslogFormatter fmt : formats) {
    ...
    public JsonEvent extractEvent(ChannelBuffer in) {
        byte b = 0;
        JsonEvent e = null;
